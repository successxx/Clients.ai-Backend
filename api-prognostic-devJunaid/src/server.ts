...
import "./jobs/emailScheduler"; // Starts the cron job
import "./queues/emailQueue"; // Initializes job queue
import { StatsRoutes } from "./routes/statsRoutes";
import { getTokenFromHeader, readJwtClaims } from "./utils/jwtUtil";
import { IJwt } from "./interfaces/IJwt";
import { verifyPaymentStatus } from "./middleware/checkSubscriptionStatus";
import "./jobs/chargeLeadsJob";
...
const init = async () => {
  ...
  server.ext("onPostAuth", async (request: Request, h: ResponseToolkit) => {
    try {
      // Check if the route has the "authRequired" tag
      if (!request.route.settings.tags?.includes("authRequired")) {
        return h.continue; // Skip middleware for routes without the tag
      }
      ...
      await verifyPaymentStatus(userId);

      return h.continue;
    } catch (error) {
      console.error("Error in onPostAuth middleware:", error);
      return Boom.unauthorized("Payment verification failed");
    }
  });
  ...
  server.route([...prefixRoutes("/stats", "Stats", StatsRoutes)]);
  server.route({
    method: "GET",
    path: "/",
    options: {
